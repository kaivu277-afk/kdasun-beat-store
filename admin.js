(() => {
  ...
  function load(index) {
    ...
    ["title","genre","bpm","key","price","mood"].forEach(key => { $(`#${key}`).value=beat[key] ?? ""; });
  ...
  function saveCurrent() {
    ...
    Object.assign(beats[current], { title, genre, bpm:Math.max(1,Number($("#bpm").value)||90), key:$("#key").value.trim()||"C", price:Math.max(0,Number($("#price").value)||0), mood:$("#mood").value.trim(), description:$("#desc").value.trim() });
